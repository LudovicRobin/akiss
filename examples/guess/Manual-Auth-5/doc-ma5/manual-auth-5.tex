...
%\resizebox{0.55\textwidth}{!}
{
  ...
  \begin{msc}{}
   ...
   \nextlevel[-1]
      ...
      \mess{$m_1 = k$}{A}{B}
      \nextlevel[1,4]
      \mess{$m_2 = \shash(d_A,k)\qquad\qquad\qquad$}{A}{H}
      \nextlevel[1,4]
      \mess{$m_2 = \shash(d_A,k)\qquad\qquad\qquad$}{H}{B}
      \nextlevel[1,5]
      \action*{check $m_2 \stackrel{?}{=} \shash(d_B,m_1)$}{B}
      \action*{check $m_2 \stackrel{?}{=} \shash(d_B,k)$}{A}
      \nextlevel[1,5]
      \action*{check $\hash(m_1)=m_0$}{B}
      \nextlevel[3]
      \mess{accept/reject}{A}{H}
      \mess{accept/reject}{B}{H}

    \end{msc}
}
    
    \caption{Manual authentication mechanism 5}
\end{figure}
...
\multirow{2}{*}{one session} & Auth. A & 336 & 13s & ok \\ 
& Auth. B & 246 & $<$ 10s & ok \\  \hline
\multirow{2}{*}{without start} & Auth. A & 271/480 & $<$ 6s & attack \\ 
& Auth. B & 399/1098 & $<$ 10s & attack \\  \hline
    without start & Auth. A & 480 & $<$ 3s & ok\\ 
+ h strong & Auth. B & 1098 & $<$ 7s & ok \\  \hline
%\multirow{2}{*}{two sessions B in par.} & Auth. A & & & attack  \\ 
%& Auth. B & & & attack  \\  \hline
\multirow{2}{*}{two sessions} & Auth. A & ? & ? & stack overflow \\ 
& Auth. B & ? & ? & stack overflow \\  \hline
\end{tabular}
\end{center}
